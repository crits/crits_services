...
urlpatterns = patterns('taxii_service.views',
    ...
    (r'^taxii_agent/preview/(?P<poll_id>[\S.]+)/(?P<page>[\S.]+)/(?P<mult>[\S.]+)/$', 'get_import_preview'),
    (r'^taxii_agent/select/(?P<poll_id>[0-9]+\.[0-9]+)/(?P<select>[01x]*)/$', 'select_deselect_all'),
    (r'^taxii_agent/select/(?P<block_id>[\S.]+)/(?P<select>[\S.]+)/$', 'select_deselect_block'),
