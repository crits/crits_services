...
class CRITsScript(CRITsBaseScript):
    ...
    def run(self, argv):
        parser = OptionParser()
        parser.add_option('-b', '--bucket-list', action='store', dest='bucket', type='string', help='bucket list name')
        parser.add_option("-o", "--output-file", action="store", dest="outfile", type="string", help="output archive file (no extension)")
        (opts, args) = parser.parse_args(argv)
        samples = mongo_connector(settings.COL_SAMPLES)
        sample_bucket = samples.find({"bucket_list": opts.bucket})
        if opts.bucket and opts.outfile:
            ...
            try:
                ...
                sys.exit(1)
        count = sample_bucket.count()
        if count <= 0:
            print ("No matching bucket name found!")
            sys.exit(1)
        for bucket in sample_bucket:
            m = bucket['md5']
            f = bucket['filename']
            s = get_file(m)
